...
// FileOutbox is the shipped durable Outbox: each buffered signing-evidence event
// is one JSON file in a spool directory, so events written on the request path
// survive a crash/redeploy and are re-published by the drainer after restart
// (construct the FileOutbox over the same directory). Use it — or a DB-backed
// Outbox — in production instead of the non-durable MemoryOutbox default.
//
...
// between Dequeue and a successful publish, which the drainer narrows further by
// re-enqueueing on cancellation. Corrupted or unreadable spool files are skipped,
// not fatal.
//
// It is safe for concurrent Enqueue with a single Dequeue consumer (the
// Drain/Flush contract). The directory must be private to one Emitter instance;
// two processes (or two Outboxes — e.g. an eIDAS-audit and a GDPR-audit spool)
// must not share a spool directory.
...
func NewFileOutbox(dir string, capacity int) (*FileOutbox, error) {
	...
	if err := os.MkdirAll(dir, 0o700); err != nil {
		return nil, fmt.Errorf("eidas-audit: create outbox spool dir: %w", err)
	}

	entries, err := os.ReadDir(dir)
	if err != nil {
		return nil, fmt.Errorf("eidas-audit: read outbox spool dir: %w", err)
	}
...
func (o *FileOutbox) Enqueue(rec *broker.Envelope) error {
	...
	if err != nil {
		return fmt.Errorf("eidas-audit: marshal outbox record: %w", err)
	}
	...
	if err := os.WriteFile(tmp, data, 0o600); err != nil {
		return fmt.Errorf("eidas-audit: write outbox record: %w", err)
	}

	if err := os.Rename(tmp, filepath.Join(o.dir, name)); err != nil {
		_ = os.Remove(tmp)

		return fmt.Errorf("eidas-audit: commit outbox record: %w", err)
	}
