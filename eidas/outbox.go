...
// DefaultOutboxCapacity is the default maximum number of buffered signing-evidence
// events the local fallback buffer holds before Enqueue reports ErrOutboxFull.
const DefaultOutboxCapacity = 1024

// ErrOutboxFull is returned by Enqueue when the buffer is at capacity.
var ErrOutboxFull = errors.New("eidas-audit: outbox is full")

// Outbox is the local buffer signing-evidence events are written to before the
// background drainer publishes them to the broker (durable-first emission). The
// default MemoryOutbox is per-pod and NON-DURABLE — buffered events are lost on
// a crash/redeploy — so production services should supply the shipped disk-backed
// FileOutbox (or a DB-backed implementation) via Options, so buffered legal
// evidence survives a crash/redeploy. Implementations must be safe for concurrent
